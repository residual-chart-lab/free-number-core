...
\(S\oplus I_{\mathbb H}\). Note 25 then constructs transports between all
different adjacent spacing words from their stationary shared edges. The
unique core loop is flat, while the exceptional quaternion is a
\(212\)-supported defect killed by both incident hinges.
