...
## 11. Next target — updated by Notes 14–23
...
1. enumerate the multiple admissible transported anchors on each exact
   reduced \(n=7\) word and compute their transition maps;
2. factor those maps into elementary internal spectator passages and test
   whether Note 20's \(\theta\) generates them;
3. compose the exact transitions around genuine closed paths and determine
   whether any nonconfluence factors through \(\kappa_{212}\);
4. extend the internal-word construction to three spectators and prove a
   uniform word theorem;
5. prove all-\(n\) middle exactness and only then seek a third differential.

The immediate mathematical target is therefore not curvature. It is the
transition graph among quotient coordinates that already exist. A curvature
candidate becomes defined only after that graph contains a closed comparison
of transport paths.
