...
## 11. Next target — updated by Notes 14–19
...
even length on odd-even-even-odd supports. At that stage their relation to
\(K_4\) remained open. Note 18 removes one ambiguity from that problem:
\(K_4=\iota(\mathbb H)\) now has the canonical coordinate
\(\nu=\frac14\iota^*\), an orthogonal complement, and an explicit
six-edge incidence table. Note 19 then resolves the central \(n=6\)
comparison: \(\chi_6=\beta\Omega_6\), but
\(\beta(K_4\otimes V)=0\), so the surviving cap quaternion is born from
\(W_{12}\otimes V\).
...
1. transport \(\omega_5\) through spectator variables with an explicit
   ...
   decoders;
2. compress Note 19's \(n=6\) normalized quotient into a primitive formula
   and extend it to the complete \(n=6,7\) decorated edge-image profiles;
