...
This note fixes the general notation for probe-depth response profiles in
quaternionic boundary-word spaces.

The purpose is definitional.

The coefficient theorem for the canonical vertical component is proved
separately in Note 06.
...
$$\vec r=(r_1,\ldots,r_d),\qquad 0\le r_1<r_2<\cdots<r_d\le n.$$
...
For an element $T\in B_n$, define the response component

$$\mathcal{R}_{n,d}^{\vec r}(T)(x_1,\ldots,x_d)=m_{n+d}\left(I_{\vec x}^{\vec r}T\right).$$
...
This note distinguishes the full distinct-slot profile from selected internal
components.

A selected component can prove visibility of a residual, but it is not a
replacement for the full distinct-slot profile.

## 4. Full-null and internal-null residuals

Let $\mathcal{S}_{\mathrm{full}}(n,d)$ be the set of all depth $d$ distinct slot
choices

$$\vec r=(r_1,\ldots,r_d),\qquad 0\le r_1<r_2<\cdots<r_d\le n.$$

Let $\mathcal{S}_{\mathrm{int}}(n,d)$ be the set of all depth $d$ distinct
internal slot choices

$$\vec r=(r_1,\ldots,r_d),\qquad 1\le r_1<r_2<\cdots<r_d\le n-1.$$
...
For $n\ge 2$, the canonical vertical component is the internal response of depth
$n-1$ obtained by inserting one probe into each internal slot:

$$\vec r_{\mathrm{vert}}=(1,2,\ldots,n-1).$$

Define

$$A_n=\mathcal{R}_{n,n-1}^{\vec r_{\mathrm{vert}}}.$$

Since there are exactly $n-1$ internal slots, $\vec r_{\mathrm{vert}}$ is the
unique depth $n-1$ internal slot choice.
...
The definition of $A_n$ contains no coefficient.

Any coefficient such as $-2$, $4$, or $-8$ is an output of the quaternionic
computation, not part of the definition.
...
For an element $S\in S^n_0V$, define the contraction map
...
$$A_n(S)=(-2)^{n-1}C_S\qquad(S\in S^n_0V,\ n\ge 2).$$
...
as an $\mathrm{SO}(3)$ module, the highest spin that can appear in the target is
$V_{d+1}$.
...
This explains why the canonical vertical component $A_n$ is the natural first
place to test visibility of the highest-spin residual.

This is a representation-theoretic visibility statement.

The scalar coefficient of the canonical vertical component is proved separately
in Note 06.
...
1. that the full depth $d$ response profile is equivalent to the canonical
   vertical component;
...
The current note only fixes the notation and the canonical vertical component
...
A component may vanish under one compression map without being structurally
absent.

The probe-depth response profile records how such residuals behave when the
depth of insertion is increased.
