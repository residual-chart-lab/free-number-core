...
\lstdefinelanguage{Lean}{
  morekeywords={namespace,abbrev,structure,where,inductive,def,constant,theorem,by,Prop,Nat,Type,deriving,Repr,DecidableEq,not,Exists,fun,exact,cases,simp,end},
  sensitive=true,
  morecomment=[l]{--},
  morecomment=[s]{/-}{-/},
  morestring=[b]"
...
\date{Version v0.2-pre\\2026-06-19}
...
\begin{figure}[t]
\centering
\input{figures/local_global_collapse.tikz}
\caption{Local closure does not imply global collapse. A solid arrow denotes an admissible transition; a dashed arrow denotes a residual transition.}
...
We now describe the local closure $F_1$. The purpose of this section is to isolate the local algebraic core before introducing global residual chart generation.

Let $U$ be a two-dimensional real vector space equipped with a negative definite quadratic form. We call $U$ the \textbf{primitive fold plane}. Choose an ordered orthonormal basis
\[
u,v\in U
\]
such that
...
u^2=v^2=-e_0,
\]
where $e_0$ is the distinguished local unit.

We impose the anti-commutation relation
\[
uv=-vu.
\]
...
Moreover,
\[
K^2=(IJ)(IJ)=I(JI)J=I(-IJ)J=-I^2J^2=-e_0.
\]
...
\]
Thus the local closure is
\[
F_1=\operatorname{span}_{\mathbb R}\{e_0,I,J,K\}.
\]

The three pure directions $I,J,K$ should not be confused with three independent primitive generators. The primitive fold plane is two-dimensional:
\[
\dim U=2.
\]
The third pure direction arises from the product of the two primitive directions:
\[
K=IJ.
\]
Equivalently, $K$ may be regarded as the exterior bivector component associated with
\[
\Lambda^2 U.
\]
Since
\[
\dim \Lambda^2 U=\frac{2(2-1)}{2}=1,
\]
the pure local direction space has dimension
\[
\dim(U\oplus\Lambda^2U)=2+1=3.
\]
Hence
\[
F_1=\mathbb R e_0\oplus U\oplus \Lambda^2U
\]
has dimension four.

\begin{proposition}[Local quaternionic form]
The local closure $F_1$ is isomorphic, as a real algebra, to the quaternion algebra $\mathbb H$.
\end{proposition}
...
by
\[
\Phi(1)=e_0,\qquad \Phi(i)=I,\qquad \Phi(j)=J,\qquad \Phi(k)=K.
\]
...
ij=k,\qquad jk=i,\qquad ki=j,
\]
with reversed products acquiring a minus sign.

By construction, $I,J,K$ satisfy the corresponding relations:
\[
I^2=J^2=K^2=-e_0,
\]
\[
...
JI=-K,\qquad KJ=-I,\qquad IK=-J.
\]
Therefore $\Phi$ preserves multiplication. Since both $\mathbb H$ and $F_1$ have real bases of cardinality four, $\Phi$ is a linear bijection. Hence
\[
F_1\cong_{\mathbb R\text{-alg}}\mathbb H.
\]
\end{proof}

\begin{corollary}[Local but not global quaternionicity]
The local closure $F_1$ has quaternionic algebraic form. However, this does not imply that the global free-number structure $F$ is itself a quaternion algebra.
\end{corollary}

In the global construction, many local copies $F_{1,\alpha}$ may each be quaternionic while remaining non-identical unless connected by admissible transitions. Thus quaternionicity is a local closure property, not a global collapse principle.
...
A lightweight measure is defined by
\[
weight(X)=2\cdot openGenCount(X)+termSize(X).
\]
For the restricted kernel, every one-step reduction strictly decreases this measure:
\[
...
The present version separates proved components, definitional components, and deferred components. Table~\ref{tab:axioms} lists the abstract assumptions used in the minimal formal kernel and indicates their intended role in later versions.
...
\bottomrule
\end{tabular}
\caption{Axioms and deferred components in v0.2-pre.}
\label{tab:axioms}
\end{table}
